...
export function RegisterModal({ isOpen, onClose, onSwitchToLogin, onSuccess }: RegisterModalProps) {
  ...
  return (
    <div 
      ...
      onClick={handleClose}
    >
      <div className="min-h-full flex items-start sm:items-center justify-center p-4 py-8 sm:py-12">
        <div 
          className="bg-card rounded-2xl max-w-md w-full relative shadow-2xl border border-border animate-in fade-in-0 zoom-in-95 duration-200"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Sticky header with close button - always visible */}
          <div className="sticky top-0 bg-card rounded-t-2xl z-10 px-6 pt-6 pb-4 border-b border-border/50">
            <button
              type="button"
              onClick={handleClose}
              className="absolute top-4 right-4 text-muted-foreground hover:text-foreground transition-colors p-2 rounded-lg hover:bg-muted z-20"
              aria-label="Cerrar"
            >
              <X size={24} />
            </button>
            <h2 className="font-serif text-2xl font-bold text-foreground pr-10">Crear Cuenta</h2>
          </div>

          {/* Scrollable content */}
          <div className="px-6 pb-6 pt-4 max-h-[70vh] sm:max-h-none overflow-y-auto">
            {error && (
